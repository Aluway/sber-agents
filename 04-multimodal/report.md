...
**Finance Bot** - LLM-based Telegram-бот для автоматического учета доходов и расходов с поддержкой мультимодального ввода (текст, изображения).
...
- **Монолитная архитектура** - весь код в одном файле `src/bot.py` (~400 строк)
...
*Скриншот демонстрирует основные возможности бота:*
...
> **Примечание:** Для добавления дополнительных скриншотов разместите изображения в папке `screenshots/` и обновите ссылки в этом разделе.
...
**API Endpoint:**
```
http://195.209.210.250:11434/v1/chat/completions
```
...
### 5. **Итерационная разработка с AI-ассистентом**
...
- Итераций: 6
- Коммитов: ~15
- Строк кода: ~400 (bot.py)
- Строк документации: ~1,500

**Зависимости:**
- Установлено пакетов: 30
- Размер виртуального окружения: ~500 MB

**Модели:**
- На сервере: 4 модели (25 GB)
- Использовано: 2 (текст + vision)
