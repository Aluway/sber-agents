...
class Config:
    ...
    # ExchangeRate API настройки (опционально, для конвертера валют)
    EXCHANGERATE_API_KEY = os.getenv("EXCHANGERATE_API_KEY")  # Опционально, можно использовать без ключа
