...
def init_ragas_metrics():
    ...
    # max_retries=5 с exponential backoff для обработки rate limits (429 ошибок)
    langchain_llm = ChatOpenAI(
        model=config.RAGAS_LLM_MODEL, 
        temperature=0,
        max_retries=5  # Exponential backoff при 429 ошибках
    )
    ...
    # Уменьшен max_workers с 4 до 2 для снижения нагрузки на API и предотвращения 429 ошибок
    run_config = RunConfig(
        max_workers=2,  # Уменьшено с 4 для снижения rate limit ошибок
        max_wait=180,
        max_retries=3
    )
