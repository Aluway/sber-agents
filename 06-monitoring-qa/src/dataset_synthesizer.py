...
def synthesize_qa_pairs_from_pdf(chunks: List, llm_model: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    Синтез вопросов и ответов из PDF чанков через LLM
    
    Args:
        chunks: список чанков документов
        llm_model: модель для синтеза (если None, используется MODEL из конфигурации)
    ...
    # Используем модель из конфигурации, если не указана явно
    if llm_model is None:
        # Используем специальную модель для синтеза датасета, если указана, иначе основную модель
        llm_model = config.DATASET_SYNTHESIS_MODEL or config.MODEL
        logger.info(f"Using model from config: {llm_model}")
    
    # Используем OpenAI-совместимый API (OpenRouter, OpenAI, Fireworks и т.д.)
    llm = ChatOpenAI(
        model=llm_model,
        temperature=0.7,
    ...
    logger.info(f"Using LLM for synthesis: {llm_model}")
