...
def load_json_qa_pairs(data_dir: str, samples_per_file: int = 10) -> List[Dict[str, Any]]:
    """
    ...
    for json_file in json_files:
        try:
            ...
            if not isinstance(data, list):
                logger.warning(f"JSON file {json_file.name} does not contain a list, skipping")
                continue
            
            if not data:
                logger.warning(f"JSON file {json_file.name} is empty, skipping")
                continue
            ...
            logger.error(f"Error loading JSON file {json_file.name}: {e}")
            continue
...
def synthesize_qa_pairs_from_pdf(chunks: List, llm_model: str = None) -> List[Dict[str, Any]]:
    ...
    if not model:
        logger.warning("MODEL not set in config, using default 'gpt-4o'. This may not work with custom base_url (e.g., Fireworks)")
        model = "gpt-4o"
    
    # Настройка ChatOpenAI с учетом base_url и api_key из конфига
    llm_kwargs = {
        "model": model,
        "temperature": 0.7
    }
    ...
    logger.info(f"Using LLM model: {model} (base_url: {config.OPENAI_BASE_URL or 'default'})")
...
{{
  "qa_pairs": [
    ...
    for i, chunk in enumerate(chunks):
        ...
        try:
            # Используем весь чанк (он уже ограничен размером при создании)
            ...
            if not response or not hasattr(response, 'content'):
                logger.warning(f"Empty or invalid response for chunk {i}, skipping")
                continue
            
            content = response.content.strip()
            if not content:
                logger.warning(f"Empty content in response for chunk {i}, skipping")
                continue
...
def create_dataset(data_dir: str, samples_per_file: int = 10) -> List[Dict[str, Any]]:
    """
...
def upload_to_langsmith(dataset_path: str, dataset_name: str, force: bool = False):
    """
    ...
    Args:
        ...
        force: если True, автоматически удаляет существующий датасет без запроса
    """
    ...
    try:
        ...
        if not qa_pairs:
            logger.warning("Dataset is empty, nothing to upload")
            return
    except json.JSONDecodeError as e:
    ...
    try:
        ...
        if existing_datasets:
            ...
            if force:
                logger.info("Force flag set, deleting existing dataset...")
                ...
                try:
                    ...
                    logger.warning("Input not available in non-interactive mode. Use --force flag to auto-delete.")
                    return
    except Exception as e:
    ...
    for qa in qa_pairs:
        ...
        if not qa.get("question") or not qa.get("ground_truth"):
            logger.warning(f"Skipping invalid Q&A pair (missing question or ground_truth)")
            continue
    ...
    if not examples:
        logger.error("No valid examples to upload")
        return
    ...
    try:
        ...
        logger.error(f"Error uploading examples to LangSmith: {e}")
        raise
...
def main():
    ...
    parser.add_argument("--samples", type=int, default=10, help="Number of samples per file (default: 10, to get ~20 pairs)")
    parser.add_argument("--force", action="store_true", help="Force delete existing dataset in LangSmith without prompt")
    ...
    if args.upload:
        ...
        upload_to_langsmith(dataset_path, dataset_name, force=args.force)
